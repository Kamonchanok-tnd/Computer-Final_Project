...
func TestQuestionnaireGroupQuestionnaire_AllCases(t *testing.T) {
	...
	// required IDs
	t.Run("QuestionnaireGroupID is required", func(t *testing.T) {
		m := validQGQ()
	...
	t.Run("QuestionnaireID is required", func(t *testing.T) {
		m := validQGQ()
		m.QuestionnaireID = 0
		ok, err := govalidator.ValidateStruct(m)
		g.Expect(ok).To(BeFalse())
	...
	t.Run("OrderInGroup > 100000 -> range invalid", func(t *testing.T) {
		...
		g.Expect(err.Error()).To(Equal("ลำดับในกลุ่มต้องมากกว่าหรือเท่ากับ 1 และไม่เกิน 100000"))
	})
