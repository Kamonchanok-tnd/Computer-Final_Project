...
const EditMessagePage: React.FC = () => {
  ...
  const [menuMaxH, setMenuMaxH] = useState(280);
  ...
  const updateThumb = () => {
    ...
    const left = Math.max(0, btnRect.left - contRect.left + 4); // กันชน 4px
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (contentKind === "long" && !formData.articleType) return msgApi.error("กรุณาเลือกประเภทบทความ");
    ...
    <div className="w-full min-h-screen bg-slate-100 p-6 lg:p-8">
      ...
      >
        <form onSubmit={handleSubmit} noValidate className="grid grid-cols-1 gap-10 lg:grid-cols-2">
          ...
          {/* Right column (Upload + วันที่ + ปุ่ม) */}
          <div className="flex flex-col">
