...
const EditCriteriaPage: React.FC = () => {
  ...
  const [loading, setLoading] = useState(true);         // โหลดข้อมูลจากเซิร์ฟเวอร์
  const [submitting, setSubmitting] = useState(false);  // สปิน fullscreen ตอนกดบันทึกเท่านั้น
  ...
  const handleSaveAll = async () => {
    ...
    try {
      setSubmitting(true); // แสดงสปิน fullscreen ตอนบันทึก
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100">
      ...
      {/* สปิน fullscreen เฉพาะตอนบันทึก (สไตล์เดียวกับหน้า create) */}
      ...
      <div className="w-full px-4 pb-6 sm:px-6">
        <div className="w-full rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 pb-16 md:pb-6">
          <Spin spinning={loading} tip="กำลังโหลดข้อมูล...">
            {/* ── ตัวอย่าง (อยู่บนสุดของการ์ด, นอกพื้นที่สกรอลล์) ── */}
            ...
            {/* เนื้อหาที่สกรอลล์ได้ (ฟอร์ม + รายการ) */}
              ...
              {criteriaList.length === 0 ? (
                ...
                <div className="space-y-3">
                  {criteriaList.map((c, idx) => (
                    ...
                    >
                      {/* Header: ชื่อเกณฑ์ + ปุ่มลบ อยู่บรรทัดเดียวกัน */}
