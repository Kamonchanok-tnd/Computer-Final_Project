...
/* — คัดลอก DropdownSearchSelect / NumberStepper / UploadBox เหมือนหน้า Create — */
const DropdownSearchSelect: React.FC<{
  ...
  return (
    <div ref={boxRef} className={"relative " + className}>
      <button type="button" disabled={disabled} onClick={() => setOpen(v => !v)} className={fieldClass + " flex items-center justify-between " + (disabled ? "opacity-60 cursor-not-allowed " : "")}>
        <span className={selected ? "truncate" : "text-slate-400"}>
          {selected ? (
            <span className="inline-flex items-center gap-2">
              {selected.icon && (typeof selected.icon === "string" ? <img src={selected.icon} alt="" className="h-4 w-4" /> : selected.icon)}
              {selected.label}
            </span>
          ) : placeholder}
        </span>
        <svg className="h-4 w-4 shrink-0 text-slate-500" viewBox="0 0 20 20" fill="currentColor"><path d="M5.23 7.21a.75.75 0 0 1 1.06.02L10 11.06l3.71-3.83a.75.75 0 1 1 1.08 1.04l-4.25 4.39a.75.75 0 0 1-1.08 0L5.21 8.27a.75.75 0 0 1 .02-1.06z"/></svg>
      </button>
        ...
        <div className="absolute left-0 right-0 z-50 mt-2 rounded-xl border border-slate-200 bg-white shadow-xl">
          <div className="flex items-center gap-2 px-3 pt-3">
            <div className="flex items-center rounded-lg border border-slate-300 px-3 py-2 hover:border-black w-full">
              ...
              <input autoFocus placeholder="ค้นหา..." value={term} onChange={e => setTerm(e.target.value)} className="w-full bg-transparent outline-none text-sm" />
            </div>
            <button className="mr-3 rounded-lg px-2 py-2 text-sm text-slate-600 hover:bg-slate-100" onClick={() => setOpen(false)} type="button">ปิด</button>
          </div>
          ...
          <div className="max-h-72 overflow-auto p-2">
            {filtered.map(o => {
              ...
              return (
                <button key={String(o.value)} className={"flex w-full items-center justify-between rounded-lg px-3 py-2 text-left " + (active ? "bg-slate-100" : "hover:bg-slate-50")} onClick={() => { onChange(o.value); setOpen(false); }} type="button">
                  <span className="inline-flex items-center gap-2">
...
const NumberStepper: React.FC<{ value: number; onChange: (n: number) => void; min?: number; max?: number; className?: string; }> = ({ value, onChange, min = 1, max = 9999, className = "" }) => {
  const clamp = (n: number) => Math.max(min, Math.min(max, n));
  return (
    <div className={"flex w-full items-stretch gap-2 " + className}>
      <button type="button" onClick={() => onChange(clamp(value - 1))} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-slate-700 hover:border-black hover:bg-slate-50">−</button>
...
const UploadBox: React.FC<{ pictureBase64?: string; setPictureBase64: (v?: string) => void; }> = ({ pictureBase64, setPictureBase64 }) => {
  ...
  const fileToBase64 = (file: File) => new Promise<string>((resolve, reject) => { const r = new FileReader(); r.readAsDataURL(file); r.onload = () => resolve(r.result as string); r.onerror = reject; });
...
const EditQuestionnaire: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      if (!qid) {
        message.error("ไม่พบรหัสแบบทดสอบ"); navigate(-1); return;
      }
      try {
        setPageLoading(true);
        ...
        const url = ensureDataUrl(data?.picture); if (url) setPictureBase64(url);
      } catch (err) {
        console.error(err); message.error("โหลดข้อมูลไม่สำเร็จ"); navigate(-1);
      } finally {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate("/admin/editQuestionAndAnswerPage", { state: { questionnaireId: qid, quantity } });
    } catch (err) {
      console.error(err);
      message.error("บันทึกการแก้ไขไม่สำเร็จ กรุณาลองใหม่");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="min-h-screen w-full bg-slate-100 py-8">
        <div className="w-full px-6">
          <div className="w-full rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex w-full items-center justify-center py-20"><Spin /></div>
          </div>
        </div>
      </div>
    );
