...
const TimerPanel: React.FC<TimerPanelProps> = ({ playingSounds, volumes, selectedSID }) => {
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const endAtRef = useRef<number | null>(null);
  const tickRef = useRef<number | null>(null);

  const [mode, setMode] = useState<Mode>("pomodoro");
  const [durations, setDurations] = useState({ pomodoro: 20, short: 5, long: 15 });
  ...
  const saveState = (override?: Partial<any>) => {
    const payload = { mode, durations, isRunning, autoTransition, endAt: endAtRef.current, ...override };
  ...
  useEffect(() => {
    ...
    tickRef.current = window.setInterval(() => {
      ...
      if (remain <= 0) {
        ...
        saveState({ isRunning: false, endAt: null });
        playSound();
        if (autoTransition) handleAutoTransition();
      }
    }, 1000);
  ...
  const startTimer = async () => {
    ...
    try {
      ...
      // สร้าง recentSettings สำหรับเสียงที่เลือก
      const recentSettings = selectedSID ? [{ sid: selectedSID, volume: volumes[selectedSID] || 50 }] : [];
      ...
      console.log(`ASMR record saved for user ${userId}, duration ${selectedDuration} min`, recentSettings);
    } catch (err) {
      console.error("Failed to save ASMR record", err);
    }
  };

  const pauseTimer = () => { setIsRunning(false); endAtRef.current = null; saveState({ isRunning: false, endAt: null }); };
  const resetTimer = () => { setIsRunning(false); endAtRef.current = null; setTimeLeft(durations[mode] * 60); saveState({ isRunning: false, endAt: null }); };

  const handleAutoTransition = () => {
    let next: Mode = mode === "pomodoro" ? "short" : mode === "short" ? "pomodoro" : "pomodoro";
    setMode(next);
        ...
        <div className="text-center mb-4">
          ...
          <div className="flex justify-center mb-4 space-x-4">
            {(["pomodoro", "short", "long"] as const).map((m) => (
              <button
                key={m}
                onClick={() => { setIsRunning(false); endAtRef.current = null; setMode(m); saveState({ mode: m, isRunning: false, endAt: null }); }}
                className={`px-1.5 py-0.5 rounded-full text-xs transition-colors ${mode === m ? "bg-white text-black font-bold" : "bg-white/10 text-white hover:bg-white/20"}`}
              >
                {m === "pomodoro" ? "Pomodoro" : m === "short" ? "Short Break" : "Long Break"}
              </button>
            ))}
            <button onClick={() => setShowSettings(!showSettings)} className="ml-2 text-white/70 hover:text-white" title="Settings">
              <Settings size={18} />
            </button>
          </div>
          ...
          <div className="flex justify-center space-x-2">
            {!isRunning ? (
              <button onClick={startTimer} className="px-3 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors">Start</button>
            ) : (
              <button onClick={pauseTimer} className="px-3 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-colors">Pause</button>
            )}
            <button onClick={resetTimer} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors">Reset</button>
          </div>
        </div>
        ...
        {showSettings && (
          <div className="mt-4 space-y-3 text-sm text-white">
            <label className="flex items-center space-x-2">
              <input type="checkbox" checked={autoTransition} onChange={(e) => { setAutoTransition(e.target.checked); saveState({ autoTransition: e.target.checked }); }} />
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["pomodoro", "short", "long"] as const).map((key) => (
                <div key={key} className="flex flex-col items-center">
                  <label className="text-white/60 text-xs mb-1">{key === "pomodoro" ? "Pomodoro" : key === "short" ? "Short Break" : "Long Break"}</label>
                  <input type="number" min={1} max={120} value={durations[key]} onChange={(e) => {
