...
import { AiOutlineHeart, AiFillHeart, AiOutlineArrowLeft, AiOutlineEye } from "react-icons/ai";
import { getAllWordHealingMessagesForUser, likeMessage, unlikeMessage, checkIfLikedArticle, updateViewCount } from "../../../services/https/message";
...
function UserMessagePage() {
  ...
  // Polling to update data every 5 seconds
  useEffect(() => {
  ...
  return (
    <div className=" flex flex-col items-center p-6 min-h-screen bg-gradient-to-b from-white to-[#C2F4FF] dark:bg-gray-900 dark:from-gray-500 dark:to-gray-900 duration-300 transition-all">
      <HeartBackground />

      <div className="w-full max-w-4xl mb-4 flex items-center">
        <button onClick={handleBack} className="p-2 bg-[#5DE2FF] text-white rounded-full hover:bg-[#4AC5D9] transition duration-300">
      ...
      <div className="w-full max-w-4xl mb-4 flex items-center justify-between">
        ...
        <div className="inline-flex bg-gray-100 rounded-full p-1">
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition ${view === "all" ? "bg-white shadow text-gray-900" : "text-gray-600 hover:text-gray-800"}`}
          >
      ...
      {currentMessages.length === 0 ? (
        ...
        <div className={`grid gap-4 w-full items-stretch ${view === "liked" ? "md:grid-cols-2 xl:grid-cols-3 grid-col-1" : "grid-cols-1"}`}>
          {currentMessages.map((message) => (
            <div key={message.id} className={`flex flex-col bg-white rounded-xl shadow-lg lg:p-6 p-4 mx-auto dark:bg-gray-700 text-white ${view === "liked" ? "w-full" : "lg:w-3/4 xl:w-1/2 w-full"} transition-all duration-300`}>
              <h3 className="text-lg mb-4 font-bold text-gray-800 dark:text-white">{message.name}</h3>
              <img src={message.photo} alt={message.name} className="w-full h-120 object-cover rounded-lg mb-4" onError={(e) => {(e.currentTarget as HTMLImageElement).src = "/default-image.png";}} />
              ...
              <div className="flex items-center justify-between mt-auto">
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleLike(message.id)} className="text-red-500 text-3xl" aria-label={liked[message.id] ? "เลิกถูกใจ" : "ถูกใจ"}>
                    {liked[message.id] ? <AiFillHeart /> : <AiOutlineHeart />}
                  </button>
                  <span className="text-gray-700 dark:text-white">{message.no_of_like}</span>
                  <AiOutlineEye className="text-xl" style={{ height: "35px", width: "35px" }} />
            ...
            <div className="flex flex-col text-sm text-gray-700">
              <div className="flex items-center gap-2 mb-2">
                {selectedMessage.articleType && (
                  <span className="px-2 py-0.5 rounded-full text-xs bg-sky-100 text-sky-700">{selectedMessage.articleType}</span>
                )}
