...
// Card component แยกสำหรับแต่ละเสียง
...
function SoundCard({ sound, extractYouTubeID, gotoSound }: SoundCardProps) {
  ...
  return (
    ...
    >
      <div className="h-auto w-full rounded-2xl relative">
        <img src={thumbnail} alt="" className="w-full h-full object-center rounded-t-xl" />
        <button
          onClick={() => gotoSound(Number(sound.ID))}
          className="absolute bottom-[-25px] sm:button-[-20] right-3 w-15 sm:w-12 sm:h-12 h-15 bg-button-blue flex items-center justify-center rounded-full shadow-lg text-white
                     opacity-100 xl:opacity-0 scale-75 translate-y-1
    ...
    <div className="font-ibmthai">
      ...
      <div className="grid lg:grid-cols-6 sm:grid-cols-3 md:grid-cols-4 grid-cols-1 sm:gap-2 gap-2 px-4 sm:px-0">
        {filteredSounds?.map((sound) => (
          <SoundCard key={sound.ID} sound={sound} extractYouTubeID={extractYouTubeID} gotoSound={gotoSound} />
        ))}
      </div>
    </div>
  );
