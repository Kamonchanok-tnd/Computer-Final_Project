...
type QuestionRow = {
  ...
  label: string;
  type: QuestionType;
  options: OptionRow[]; // ใช้เมื่อ type เป็น choice_*
};
...
export default function AdminFeedbackFormEditor() {
  ...
  const addOption = (qid: string) =>
    setRows((r) =>
      r.map((q) =>
        q.id === qid ? { ...q, options: [...q.options, { id: crypto.randomUUID(), label: "" }] } : q
      )
    );
  ...
  // ===== Submit (PUT /admin/feedback-form)
    ...
    <div className="w-full px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-6xl">
        ...
        <form onSubmit={onSubmit} className="rounded-2xl bg-white shadow p-5 sm:p-6">
          ...
          {errMsg && (
            <div className="mt-4 rounded-lg bg-red-50 text-red-700 text-sm px-3 py-2">{errMsg}</div>
          )}
          {okMsg && (
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={() =>
                setRows([{ id: crypto.randomUUID(), label: "", type: "rating", options: [] }])
              }
