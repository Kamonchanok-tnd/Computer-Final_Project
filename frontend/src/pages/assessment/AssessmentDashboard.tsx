...
import { getMyTransactions } from "../../services/https/assessment";
...
const PALETTE = [
  "#2563eb", "#10b981", "#f59e0b", "#ef4444", "#a855f7",
  "#14b8a6", "#e11d48", "#0ea5e9", "#22c55e", "#f97316",
  "#7c3aed", "#059669", "#dc2626", "#0284c7", "#16a34a",
];
...
const CustomLegend: React.FC<any> = ({ payload, compact }) => (
  <div className="w-full" style={{ fontSize: compact ? 11 : 13, lineHeight: compact ? "14px" : "16px" }}>
    <div className={`flex ${compact ? "flex-col gap-1" : "flex-wrap gap-x-4 gap-y-1"}`}>
      {payload?.map((it: any) => (
        <div key={it.value} className="flex items-center gap-1 whitespace-nowrap">
          ...
          <span className="text-slate-600 truncate" style={{ maxWidth: compact ? "180px" : "none" }}>{it.value}</span>
        </div>
      ))}
    </div>
  </div>
...
type TickProps = { x: number; y: number; payload: any; data: any[]; small?: boolean; colorOfGroup: (g: string) => string };
const RotXTick: React.FC<TickProps> = ({ x, y, payload, data, small, colorOfGroup }) => {
  ...
  const fs = small ? 9 : 11;               // ตัวเล็กลง
  const fill = colorOfGroup(group || "");  // สีตาม group
  return (
    <g transform={`translate(${x},${y})`}>
      {/* เอียง -45° และชิดขวาให้ไม่ทับกัน */}
...
function aggregateCombined(rows: TxView[], windowMin: number) {
  ...
  return Array.from(clusters.values())
    ...
    .map((c) => { delete c.__sort; delete c.__last; return c; });
}
...
const AssessmentDashboard: React.FC = () => {
  ...
  const meanScore = tx.length > 0 ? (tx.reduce((acc, t) => acc + (t.score || 0), 0) / tx.length).toFixed(1) : "0.0";
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="rounded-2xl p-5 bg-green-100/70 flex items-center gap-4">
          <div className="rounded-full bg-white/70 p-3 shadow"><Activity className="w-6 h-6 text-emerald-600" /></div>
          <div>
            <p className="text-slate-600 text-sm">การเข้าทำแบบทดสอบ</p>
            <p className="text-2xl font-semibold text-emerald-700">{totalAttempts} <span className="text-base font-normal">ครั้ง</span></p>
          </div>
        </div>

        <div className="rounded-2xl p-5 bg-violet-100/70 flex items-center gap-4">
          <div className="rounded-full bg-white/70 p-3 shadow"><BarChart3 className="w-6 h-6 text-violet-600" /></div>
          <div>
            <p className="text-slate-600 text-sm">แบบทดสอบที่เคยทำ</p>
            <p className="text-2xl font-semibold text-violet-700">{uniqueTests.length}</p>
          </div>
        </div>

        <div className="rounded-2xl p-5 bg-sky-100/70 flex items-center gap-4">
          <div className="rounded-full bg-white/70 p-3 shadow"><Award className="w-6 h-6 text-sky-600" /></div>
          <div>
            <p className="text-slate-600 text-sm">คะแนนเฉลี่ย</p>
            <p className="text-2xl font-semibold text-sky-700">{meanScore}</p>
          </div>
        </div>

        <div className="rounded-2xl p-5 bg-cyan-100/70 flex items-center gap-4">
          <div className="rounded-full bg-white/70 p-3 shadow"><Calendar className="w-6 h-6 text-cyan-600" /></div>
          <div className="truncate">
            <p className="text-slate-600 text-sm">รายการล่าสุด</p>
            {latest ? (
              <>
                <p className="text-cyan-700 font-medium truncate">{latest.description}</p>
                <p className="text-slate-500 text-xs">{latest.dateLabel} • {latest.group || "-"}</p>
              </>
      ...
      <div className="rounded-2xl bg-white shadow-sm border border-slate-100">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-slate-100">
          <div className="space-y-1">
            <h3 className="font-semibold text-slate-800">{view === "single" ? "กราฟรายแบบทดสอบ" : "กราฟรวมหลายแบบทดสอบ"}</h3>
          ...
          <div className="relative">
            <div className="flex items-center bg-cyan-400/90 rounded-full p-1 gap-1">
              <button onClick={() => setView("single")}
                className={`flex items-center gap-2 px-3 py-2 rounded-full text-sm transition-all ${view === "single" ? "bg-white text-slate-900 shadow" : "text-white/90 hover:text-white"}`}>
                <FileText size={16} /> รายแบบทดสอบ
              </button>
              <button onClick={() => setView("combined")}
                className={`flex items-center gap-2 px-3 py-2 rounded-full text-sm transition-all ${view === "combined" ? "bg-white text-slate-900 shadow" : "text-white/90 hover:text-white"}`}>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 px-4 pt-3">
          {view === "single" ? (
            <>
              <select className="px-3 py-2 rounded-xl border border-slate-200 text-sm" value={selectedTest} onChange={(e) => setSelectedTest(e.target.value)}>
                {uniqueTests.map((n) => (<option key={n} value={n}>{n}</option>))}
              </select>
              <select className="px-3 py-2 rounded-xl border border-slate-200 text-sm" value={rangeOne} onChange={(e) => setRangeOne(e.target.value as RangeKey)}>
            ...
            <div className="flex items-center gap-2">
              ...
              <select className="px-3 py-2 rounded-xl border border-slate-200 text-sm" value={rangeAll} onChange={(e) => setRangeAll(e.target.value as RangeKey)}>
                {RANGE_OPTIONS.map((o) => (<option key={o.key} value={o.key}>{o.label}</option>))}
              </select>
            </div>
          )}
          ...
          {view === "single" ? (
            dataOne.length === 0 ? (
              ...
              <ResponsiveContainer key={`single-${chartW}`} width="100%" height="100%">
                <LineChart data={dataOne} margin={{ top: 10, right: isVeryNarrow ? 12 : 20, left: isVeryNarrow ? 6 : 12, bottom: 8 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis
                    dataKey="_key"
                    scale="point"
                    interval={0}                          // แสดงทุกวันที่
                  ...
                  <YAxis />
                  <Tooltip
                    formatter={(v: any, name) => (name === "score" ? [`${v}`, "คะแนน"] : v)}
                    labelFormatter={(key) => {
                      const item = dataOne.find((d) => d._key === key);
                      return item ? `${item.dateLabel} (${item.group || "-"})` : "";
                    }}
                  />
            ...
            <>
              {/* กราฟรวม (Legend อยู่ข้างล่าง) */}
              <ResponsiveContainer key={`all-${chartW}-${seriesCount}`} width="100%" height={chartHeight - (legendH + (isVeryNarrow ? 8 : 10))}>
                <LineChart data={dataAll} margin={{ top: 10, right: isVeryNarrow ? 12 : 20, left: isVeryNarrow ? 6 : 12, bottom: 8 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis
                    dataKey="_key"
                    scale="point"
                  ...
                  />
                  {seriesNames.map((s, i) => (
                    <Line key={s} type="monotone" dataKey={s} stroke={colorFor(i)} strokeWidth={2}
                          dot={{ r: isVeryNarrow ? 1 : (isNarrow ? 1.5 : 2) }}
                          activeDot={{ r: isVeryNarrow ? 2.5 : (isNarrow ? 3 : 4) }} connectNulls />
                  ))}
                </LineChart>
              </ResponsiveContainer>

              {/* Legend แยกลงมาข้างล่าง */}
